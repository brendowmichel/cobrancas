// SISTEMA DE GESTAO DE COBRANCAS - Apps Script v31
...
<body class="min-h-screen" style="background:var(--md-surface);color:var(--md-on-surface);-webkit-font-smoothing:antialiased">
    ...
    <header class="flex items-center justify-between mb-8">
      ...
      <div style="display:flex;align-items:center;gap:.625rem;position:relative">
        <div style="position:relative">
          ...
          <div id="import-menu" style="display:none;position:absolute;right:0;top:calc(100% + .625rem);width:380px;background:var(--md-surface-container-high);border-radius:20px;box-shadow:var(--md-shadow-3);padding:1.25rem 1.375rem;z-index:100">
            <p style="font-size:.7rem;font-weight:600;color:var(--md-on-surface-variant);text-transform:uppercase;letter-spacing:.08em;margin:0 0 .875rem">Importação</p>
            <div style="display:flex;flex-direction:column;gap:.875rem">
              <div>
                ...
                <input id="arquivo-excel" type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                  style="display:block;width:100%;font-size:.8125rem;color:var(--md-on-surface-variant);cursor:pointer"
                  class="file:mr-3 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-medium file:cursor-pointer file:bg-[var(--md-secondary-container)] file:text-[var(--md-on-secondary-container)]">
              </div>
              <div style="display:flex;gap:.5rem;flex-wrap:wrap">
                <button type="button" onclick="importarArquivoExcel()" class="btn-primary">Importar Excel</button>
                <button type="button" onclick="importarAbaPadrao()" class="secondary">Importar aba</button>
                <button id="btn-preparar" type="button" onclick="prepararAbas()" class="secondary" hidden>Preparar abas</button>
              </div>
    ...
    <div class="modal">
      ...
      <div class="form-grid">
        ...
        <div>
          ...
          <select id="status-cadastro-select" class="md-field">
  ...
  <script>
    (function () {
      ...
      function attach() {
        var els = document.querySelectorAll(".btn-primary,.btn-secondary,.secondary,.cliente-row");
  ...
  <script>
    ...
    function renderGraficos(inadimplentes) {
      ...
      if (el1) el1.innerHTML = buildGrafico_("STATUS DE CONTATO", total + " cliente(s) c/ 2+ duplicatas", CONTATO_DEFS, contatoCounts, contatoTotals);
      if (el2) el2.innerHTML = buildGrafico_("STATUS DE CADASTRO", totalCadastro + " com status especial (de " + total + ")", CADASTRO_DEFS, cadastroCounts, cadastroTotals);
    }

    function buildGrafico_(titulo, subtitulo, defs, counts, totals) {
      ...
      return "<span style='display:block;font-size:.7rem;font-weight:600;color:#9ca3af;text-transform:uppercase;letter-spacing:.08em'>" + titulo + "</span>" +
        ...
        "<div style='display:flex;flex-wrap:wrap;gap:6px 14px'>" + legendHtml + "</div>";
    }
