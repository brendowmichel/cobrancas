// SISTEMA DE GESTAO DE COBRANCAS - Apps Script v63
// Interface em Google Apps Script com migracao da base para Supabase/Postgres.

const APP_VERSION = "v63";
const SUPABASE_HASH_RECONCILIATION_BATCH_SIZE = 15;
...
const SUPABASE_TABLES = {
  ...
  HISTORICO_STATUS: "cobranca_historico_status",
  DASHBOARD_CLIENTES: "cobranca_dashboard_clientes"
};
...
function onOpen() {
  const menu = SpreadsheetApp.getUi().createMenu("Cobrancas")
    .addItem("Configurar Supabase", "configurarSupabase")
    .addItem("Testar Supabase", "testarSupabase")
    .addItem("Diagnosticar Supabase", "diagnosticarSupabase");

  if (!isSupabaseConfigurado_()) {
    menu
      .addSeparator()
...
function configurarSupabase() {
  ...
  const urlPrompt = ui.prompt(
    "Configurar Supabase",
    "Cole a Project URL do Supabase (ex.: https://xxxx.supabase.co)",
  ...
  const keyPrompt = ui.prompt(
    "Configurar Supabase",
    "Cole a API key. Use uma chave server-side apenas em PropertiesService, nunca no HTML.",
    ui.ButtonSet.OK_CANCEL
  );
  if (keyPrompt.getSelectedButton() !== ui.Button.OK) return;

  salvarConfigSupabase(urlPrompt.getResponseText(), keyPrompt.getResponseText());
  SpreadsheetApp.getActive().toast("Supabase configurado - " + APP_VERSION, "Cobrancas", 5);
}

function salvarConfigSupabase(url, apiKey) {
  const normalizedUrl = normalizarSupabaseUrl_(url);
  const normalizedKey = String(apiKey || "").trim();

  if (!normalizedUrl) {
    throw new Error("URL do Supabase invalida. Use o formato https://xxxx.supabase.co. Recebido: " + resumirEntradaConfig_(url));
  }
  if (!normalizedKey) {
    throw new Error("API key do Supabase nao informada.");
  }

  PropertiesService.getScriptProperties().setProperties({
    [SUPABASE_PROPS.URL]: normalizedUrl,
    [SUPABASE_PROPS.API_KEY]: normalizedKey
  }, true);

  return { ok: true, url: normalizedUrl };
}

function normalizarSupabaseUrl_(url) {
  const raw = String(url || "")
    .replace(/[\u200B-\u200D\uFEFF]/g, "")
    .replace(/^["'`]+|["'`]+$/g, "")
    .trim();
  const match = raw.match(/(?:https?:\/\/)?([a-z0-9.-]+\.supabase\.co)(?:[\/\s?#].*)?$/i) ||
    raw.match(/(?:https?:\/\/)?([a-z0-9.-]+\.supabase\.co)/i);
  return match ? ("https://" + match[1].toLowerCase()).replace(/\/+$/, "") : "";
}

function resumirEntradaConfig_(value) {
...
function testarSupabase() {
  const resposta = supabaseGet_(SUPABASE_TABLES.DASHBOARD_CLIENTES, {
    select: "cliente_id,cliente,cnpj_cpf,valor_total",
    limit: "1"
  });

  const qtd = Array.isArray(resposta) ? resposta.length : 0;
  const msg = "Supabase OK. Registros retornados: " + qtd;
  try {
    SpreadsheetApp.getActive().toast(msg, "Cobrancas", 5);
  } catch (e) {
    Logger.log(msg);
  }
  return { ok: true, registros: qtd, amostra: resposta };
}

function diagnosticarSupabase() {
  const hojeIso = formatDate_(new Date());
  const viewRows = supabaseGet_(SUPABASE_TABLES.DASHBOARD_CLIENTES, {
    select: "cliente_id,cliente,cnpj_cpf,duplicatas_vencidas,valor_total,maior_atraso",
    limit: "10000"
  }) || [];

  const titulosAbertos = supabaseGet_(SUPABASE_TABLES.TITULOS, {
    select: "id_interno,cliente,cnpj_cpf,vencimento,valor_a_receber,ativo_na_ultima_importacao",
    valor_a_receber: "gt.0",
    limit: "10000"
  }) || [];

  const hoje = parseIsoDate_(hojeIso);
  const titulosVencidos = titulosAbertos.filter(function (titulo) {
    if (titulo.ativo_na_ultima_importacao === false) return false;
    if (String(titulo.ativo_na_ultima_importacao).toLowerCase() === "false") return false;
    const vencimento = parseIsoDate_(titulo.vencimento);
    return Boolean(vencimento && diasEntre_(vencimento, hoje) > 0);
  });

  const resultado = {
    ok: true,
    hoje: hojeIso,
    view_dashboard: viewRows.length,
    titulos_abertos: titulosAbertos.length,
    titulos_vencidos_gas: titulosVencidos.length,
    primeira_view: viewRows[0] || null,
    primeiro_titulo_vencido: titulosVencidos[0] || null
  };

  const msg = "Supabase: view " + resultado.view_dashboard +
    " | abertos " + resultado.titulos_abertos +
    " | vencidos " + resultado.titulos_vencidos_gas;
  Logger.log(JSON.stringify(resultado, null, 2));
  try {
    SpreadsheetApp.getActive().toast(msg, "Cobrancas", 8);
  } catch (e) {
    Logger.log(msg);
  }
  return resultado;
...
function diagnosticarTitulosClienteSupabase(documento) {
  if (!isSupabaseConfigurado_()) {
    throw new Error("Supabase nao configurado.");
  }

  const cnpjCpf = normalizeDocumento_(documento || "06922577000137");
  if (!cnpjCpf) {
    throw new Error("Informe um CNPJ/CPF valido para diagnosticar os titulos.");
  }

  const hoje = parseIsoDate_(formatDate_(new Date()));
  const titulos = supabaseGet_(SUPABASE_TABLES.TITULOS, {
    select: [
      "id_interno",
      "hash_identificacao",
      "cliente",
  ...
  const detalhes = titulos.map(function (titulo) {
    const vencimento = parseIsoDate_(titulo.vencimento);
    const ativo = titulo.ativo_na_ultima_importacao !== false &&
      String(titulo.ativo_na_ultima_importacao).toLowerCase() !== "false";
    const valorAberto = parseNumber_(titulo.valor_a_receber);
    const diasAtraso = vencimento ? diasEntre_(vencimento, hoje) : 0;

    return {
      numero: titulo.numero || "",
      vencimento: titulo.vencimento || "",
      valor_a_receber: valorAberto,
      ativo_na_ultima_importacao: ativo,
      vencido: Boolean(ativo && valorAberto > 0 && vencimento && diasAtraso > 0),
      dias_atraso: diasAtraso,
      status_cobranca: titulo.status_cobranca || "",
      origem_arquivo: titulo.origem_arquivo || "",
      data_ultima_importacao: titulo.data_ultima_importacao || "",
      hash_identificacao: titulo.hash_identificacao || ""
    };
  });
  ...
  const resultado = {
    versao: APP_VERSION,
    cnpj_cpf: cnpjCpf,
    titulos_encontrados: detalhes.length,
    titulos_ativos: detalhes.filter(function (titulo) { return titulo.ativo_na_ultima_importacao; }).length,
    titulos_vencidos_ativos: detalhes.filter(function (titulo) { return titulo.vencido; }).length,
    valor_vencido_ativo: arredondar2_(detalhes
      .filter(function (titulo) { return titulo.vencido; })
      .reduce(function (total, titulo) { return total + titulo.valor_a_receber; }, 0)),
    titulos: detalhes
  };
  ...
  try {
    SpreadsheetApp.getActive().toast(
      "Cliente " + cnpjCpf + ": " + resultado.titulos_encontrados +
      " titulos | " + resultado.titulos_vencidos_ativos + " vencidos ativos",
      "Diagnostico Supabase",
      10
    );
  } catch (e) {
    Logger.log("Diagnostico executado sem planilha ativa.");
  }
...
function getSupabaseConfig_() {
  ...
  const apiKey = props.getProperty(SUPABASE_PROPS.API_KEY);

  if (!url || !apiKey) {
    throw new Error("Supabase nao configurado. Execute configurarSupabase() ou salvarConfigSupabase(url, apiKey).");
  }

  return { url: url.replace(/\/+$/, ""), apiKey: apiKey };
}

function isSupabaseConfigurado_() {
  const props = PropertiesService.getScriptProperties();
  return Boolean(props.getProperty(SUPABASE_PROPS.URL) && props.getProperty(SUPABASE_PROPS.API_KEY));
}

function supabaseGet_(resource, query) {
  return supabaseRequest_("get", resource, query || null, null, null);
}

function supabasePost_(resource, payload, query, prefer) {
  return supabaseRequest_("post", resource, query || null, payload, prefer || "return=representation");
}

function supabasePatch_(resource, payload, query, prefer) {
  return supabaseRequest_("patch", resource, query || null, payload, prefer || "return=representation");
}

function supabaseUpsert_(resource, payload, conflictColumns, prefer) {
  const query = conflictColumns ? { on_conflict: conflictColumns } : null;
  const preferHeader = prefer || "resolution=merge-duplicates,return=representation";
  return supabaseRequest_("post", resource, query, payload, preferHeader);
}

function supabaseRequest_(method, resource, query, payload, prefer) {
  const config = getSupabaseConfig_();
  const url = buildSupabaseUrl_(config.url, resource, query);
  const headers = {
  ...
  if (status < 200 || status >= 300) {
    throw new Error("Erro Supabase " + status + ": " + body);
  }

  return body ? JSON.parse(body) : null;
}

function buildSupabaseUrl_(baseUrl, resource, query) {
  const cleanResource = String(resource || "").replace(/^\/+/, "");
  let url = baseUrl + "/rest/v1/" + cleanResource;
  const queryString = buildQueryString_(query);
...
function buildQueryString_(query) {
  ...
  return Object.keys(query)
    .filter(function (key) {
      return query[key] !== null && query[key] !== undefined && query[key] !== "";
    })
    .map(function (key) {
...
function getDashboardCobrancas() {
  if (isSupabaseConfigurado_()) {
    return getDashboardCobrancasSupabase_();
  }
  return getDashboardCobrancasSheets_();
}

function getDashboardCobrancasSheets_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
...
function getDashboardCobrancasSupabase_() {
  const hojeIso = formatDate_(new Date());
  const clientesDashboard = supabaseGet_(SUPABASE_TABLES.DASHBOARD_CLIENTES, {
    select: [
      "cliente_id",
      "cnpj_cpf",
      "cliente",
  ...
  titulos.forEach(function (titulo) {
    if (titulo.ativo_na_ultima_importacao === false) return;
    if (String(titulo.ativo_na_ultima_importacao).toLowerCase() === "false") return;

    const valorAberto = parseNumber_(titulo.valor_a_receber);
    if (valorAberto <= 0) return;

    const vencimento = parseIsoDate_(titulo.vencimento);
    const diasAtraso = vencimento ? diasEntre_(vencimento, parseIsoDate_(hojeIso)) : 0;
    if (!vencimento || diasAtraso <= 0) return;

    totalVencido += valorAberto;
    qtdVencidas += 1;
    maiorAtraso = Math.max(maiorAtraso, diasAtraso);
    adicionarCobrancaAoGrupo_(grupos, titulo, valorAberto, diasAtraso);
  });

  const dashboardPorDoc = {};
  clientesDashboard.forEach(function (item) {
    const doc = item.cnpj_cpf || "";
    if (doc) dashboardPorDoc[doc] = item;
  });

  if (!Object.keys(grupos).length && clientesDashboard.length) {
    clientesDashboard.forEach(function (item) {
      const doc = item.cnpj_cpf || item.cliente_id || item.cliente || "";
      if (!doc) return;
      const valorTotal = parseNumber_(item.valor_total);
      const maior = Number(item.maior_atraso || 0);
      grupos[doc] = {
        id: doc,
        cliente: item.cliente || "SEM CLIENTE",
        cnpj_cpf: item.cnpj_cpf || "",
        duplicatas_vencidas: Number(item.duplicatas_vencidas || 0),
        valor_total: arredondar2_(valorTotal),
        maior_atraso: maior,
        responsavel: item.responsavel || "",
        status_negociacao: "",
        duplicatas: [],
        detalhe_indisponivel: true
      };
      totalVencido += valorTotal;
      qtdVencidas += Number(item.duplicatas_vencidas || 0);
      maiorAtraso = Math.max(maiorAtraso, maior);
    });
  }
  ...
  const inadimplentes = Object.keys(grupos).map(function (key) {
    const grupo = grupos[key];
    const cadastro = dashboardPorDoc[grupo.cnpj_cpf] || {};
    aplicarSupabaseClienteAoGrupo_(grupo, cadastro);
    grupo.duplicatas = agruparDuplicatasPorRps_(grupo.duplicatas);
    grupo.duplicatas_vencidas = grupo.duplicatas.length || grupo.duplicatas_vencidas || 0;
    qtdVencidasAgrupadas += grupo.duplicatas_vencidas;
    grupo.valor_total = arredondar2_(grupo.valor_total);
    if (grupo.duplicatas.length) {
      grupo.maior_atraso = Math.max.apply(null, grupo.duplicatas.map(function (item) {
        return item.dias_atraso;
      }));
    }
  ...
  return {
    ...
    fonte_dados: "supabase",
    kpis: {
      total_aberto: arredondar2_(totalVencido),
      total_vencido: arredondar2_(totalVencido),
      total_protestado: arredondar2_(inadimplentes
        .filter(function(g) { return g.status_contato === "FINALIZADO"; })
        .reduce(function(acc, g) { return acc + (g.valor_total || 0); }, 0)),
      ...
      clientes_inadimplentes: inadimplentes.length,
      duplicatas_vencidas: qtdVencidasAgrupadas,
      duplicatas_abertas: qtdVencidas,
      vence_7_dias: 0,
      maior_atraso: maiorAtraso
    },
    inadimplentes: inadimplentes.slice(0, 200),
...
function aplicarSupabaseClienteAoGrupo_(grupo, clienteCadastro) {
  const statusContato = normalizarStatusCliente_(clienteCadastro.status_contato || clienteCadastro.status_cobranca);
  const statusCadastro = normalizarStatusCadastro_(clienteCadastro.status_cadastro);
  ...
  grupo.data_agendamento_cliente = clienteCadastro.data_agendamento || "";
  grupo.data_status_cadastro_cliente = clienteCadastro.data_status_cadastro || "";
  grupo.observacao_cliente = clienteCadastro.observacao || "";
  grupo.responsavel_cliente = clienteCadastro.responsavel || "";
}
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Manrope:wght@400;500;600;700;800&family=Roboto+Flex:opsz,wght@8..144,400;8..144,500;8..144,600;8..144,700&display=swap" rel="stylesheet">
  ...
  <style type="text/tailwindcss">
    :root {
      ...
      --ui-accent:#2f7f92;
      --ui-accent-border:#6eb6c8;
      --ui-accent-container:#e7f6fa;
      --ui-accent-container-strong:#d6eff6;
      --ui-accent-text:#165769;
    ...
    body { font-family:'GT Walsheim Pro','Manrope','Roboto Flex',ui-sans-serif,system-ui,-apple-system,'Segoe UI',sans-serif; letter-spacing:0; }
    .md-icon {
    ...
    @layer components {
      ...
      .btn-primary {
        ...
        @apply inline-flex items-center justify-center gap-2 font-semibold rounded-full transition-all cursor-pointer px-6 text-sm leading-none;
        height:40px; background:var(--ui-accent); color:#ffffff; border:1px solid color-mix(in srgb, var(--ui-accent) 76%, #ffffff);
      }
      .btn-primary:hover { box-shadow:var(--md-shadow-1); background:color-mix(in srgb, var(--ui-accent) 88%, #000); }
      .btn-primary:active { filter:brightness(.95); }

      .btn-secondary, .secondary {
        position:relative; overflow:hidden;
        @apply inline-flex items-center justify-center gap-2 font-semibold rounded-full transition-all cursor-pointer px-6 text-sm leading-none;
        height:40px; background:var(--ui-accent-container); color:var(--ui-accent-text); border:1px solid color-mix(in srgb, var(--ui-accent-border) 65%, #ffffff);
      }
      .btn-secondary:hover, .secondary:hover { background:var(--ui-accent-container-strong); border-color:var(--ui-accent-border); box-shadow:var(--md-shadow-1); filter:none; }

      .secondary.small {
        @apply px-4 text-xs font-semibold rounded-full;
        height:32px; background:rgba(255,255,255,.66); color:var(--ui-accent-text);
        border:1px solid color-mix(in srgb, var(--ui-accent-border) 72%, #ffffff);
      }
      .secondary.small:hover { background:var(--ui-accent-container); border-color:var(--ui-accent-border); box-shadow:none; filter:none; }
      .icon-action {
        width:34px; min-width:34px; height:34px; padding:0;
        border-radius:999px;
      }
      .icon-action .md-icon { font-size:18px; }
      ...
      .sort-btn:hover { color:var(--ui-accent-text); }
      .sort-mark {
        width:18px; height:18px; display:inline-flex; align-items:center; justify-content:center;
        color:var(--ui-accent); line-height:1; opacity:0; transform:translateY(1px);
      }
      .sort-mark.md-icon {
        font-size:20px;
        font-variation-settings:'FILL' 1,'wght' 600,'GRAD' 0,'opsz' 20;
      }
      .sort-mark.active { opacity:1; }

      /* === Tabela =========================================== */
      .main-table { table-layout:fixed; }
      .main-table tbody td {
        @apply px-5 py-2.5 text-sm align-middle;
        color:var(--md-on-surface); border-bottom:1px solid var(--md-outline-variant);
      }
      .main-table tbody tr:last-child td { border-bottom:0; }
      .row-ag-vencido { background:color-mix(in srgb, var(--md-error-container) 82%, var(--md-surface)); }
      .row-ag-vencido td:first-child { box-shadow:inset 4px 0 0 var(--md-error); }
      ...
      .cliente-row.row-ag-vencido:hover,
      .cliente-row.row-ag-vencido.is-open {
        background:color-mix(in srgb, var(--md-error) 22%, var(--md-error-container));
      }
      .cliente-row.row-ag-vencido.is-open td:first-child {
        box-shadow:inset 4px 0 0 color-mix(in srgb, var(--md-error) 78%, #000);
      }
      ...
      /* === Status cell ====================================== */
      .status-cell { min-width:250px; max-width:340px; }
      .status-main { display:flex; align-items:center; gap:.5rem; max-width:320px; }
      .status-note {
        @apply block text-xs mt-1; color:var(--md-on-surface-variant);
        white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:300px;
      }
      .status-date { font-size:.75rem; color:var(--md-on-surface-variant); white-space:nowrap; flex-shrink:0; }
      .status-observation {
        display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical;
        white-space:normal; overflow:hidden; line-height:1.25; max-width:320px;
      }
      ...
      .status-open       { background:#ffffff; color:#51606f; border-color:#cbd4df; }
      .status-contact    { background:var(--ui-accent-container); color:var(--ui-accent-text); border-color:var(--ui-accent-border); }
      .status-scheduled  { background:#f4e9ff; color:#683f93; border-color:#d8b6f5; }
      .status-scheduled-overdue { background:#fff0ef; color:#a80712; border-color:#d4555d; }
      .status-finalizado { background:#e9f8ef; color:#09623d; border-color:#82cda2; }

      .cadastro-tag {
        @apply inline-flex items-center px-2 py-0.5 rounded-full font-bold uppercase;
        border:0; font-size:.625rem; line-height:1.05; letter-spacing:.02em;
      }
      .cadastro-suspended      { background:#fff3da; color:#7a4700; }
      .cadastro-cancelled      { background:#ffe5e1; color:#8b0d14; }
      .cadastro-protested      { background:#ffe5e1; color:#8b0d14; }
      .cadastro-permuta        { background:#dff5ef; color:#005f50; }
      .cadastro-desconsiderado { background:#eef2f7; color:#4b5868; }

      .pill { @apply inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold; border:1px solid transparent; }
      .pill-vencido  { background:#fff0ef; color:#a80712; border-color:#e19a9b; }
      .pill-aberto   { background:#ffffff; color:#51606f; border-color:#cbd4df; }
      .pill-parcial  { background:#fff7ea; color:#8a5300; border-color:#efbe70; }
      .pill-recebido { background:#e9f8ef; color:#09623d; border-color:#82cda2; }

      /* === Misc ============================================= */
      .muted { color:var(--md-on-surface-variant); }
      .num   { @apply text-right whitespace-nowrap; }
      .num-center { @apply text-center whitespace-nowrap; font-variant-numeric:tabular-nums; }
      ...
      .cnpj-line    { @apply text-xs; color:var(--md-on-surface-variant); display:inline-flex; align-items:center; gap:.375rem; flex-wrap:wrap; margin-top:1px; }
      .action-stack { @apply flex items-center gap-2 justify-end; flex-wrap:nowrap; }
      .actions      { width:112px; min-width:112px; text-align:right; }
      .version      { @apply inline-flex items-center px-3 py-1 rounded-full text-xs font-bold; background:var(--ui-accent-container); color:var(--ui-accent-text); border:1px solid color-mix(in srgb, var(--ui-accent-border) 60%, #ffffff); }
      ...
      .header-meta {
        position:relative; display:inline-flex; width:max-content; max-width:100%;
        cursor:help; outline:none;
      }
      .header-meta[data-tooltip]:not([data-tooltip=""])::after {
        content:attr(data-tooltip);
        position:absolute; left:0; top:calc(100% + .5rem); z-index:80;
        min-width:260px; max-width:440px; width:max-content;
        padding:.625rem .75rem; border-radius:14px;
        background:var(--md-inverse-surface); color:var(--md-inverse-on-surface);
        box-shadow:var(--md-shadow-3); font-size:.75rem; line-height:1.35;
        white-space:normal; opacity:0; transform:translateY(-3px);
        pointer-events:none; transition:opacity .16s, transform .16s;
      }
      .header-meta[data-tooltip]:not([data-tooltip=""]):hover::after,
      .header-meta[data-tooltip]:not([data-tooltip=""]):focus-visible::after {
      ...
      .md-card-hover:hover { border-color:color-mix(in srgb, var(--ui-accent-border) 44%, var(--md-outline-variant)); background:color-mix(in srgb, var(--ui-accent-container) 12%, var(--md-surface-container-lowest)); }
      ...
      .md-field {
        ...
        border:1px solid color-mix(in srgb, var(--ui-accent-border) 28%, var(--md-outline-variant)); border-radius:14px; outline:none;
        transition:border-color .15s, box-shadow .15s; font-family:inherit; box-sizing:border-box;
      }
      .md-field:focus { border-color:var(--ui-accent-border); box-shadow:0 0 0 2px color-mix(in srgb, var(--ui-accent-border) 28%, transparent); }
...
<body class="min-h-screen" style="background:var(--md-surface-container-low);color:var(--md-on-surface);-webkit-font-smoothing:antialiased">
  ...
  <main style="max-width:1280px;margin:0 auto;padding:2rem 1.5rem 3rem">
    ...
    <header class="flex items-center justify-between mb-8">
      <div class="flex items-center gap-3">
        <div style="width:46px;height:46px;border-radius:14px;background:var(--ui-accent-container);color:var(--ui-accent-text);border:1px solid var(--ui-accent-border);display:flex;align-items:center;justify-content:center">
          <span class="md-icon" style="font-size:26px;font-variation-settings:'FILL' 1,'wght' 500,'GRAD' 0,'opsz' 24">account_balance_wallet</span>
        </div>
        <div>
          ...
          <p id="subtitulo" class="muted header-meta" style="font-size:.8125rem;margin:.15rem 0 0">Carregando dados da base...</p>
        </div>
      </div>
    ...
    <div class="md-card" style="overflow:hidden">
      ...
      <div style="overflow-x:auto">
        <table class="main-table" style="width:100%;border-collapse:collapse;min-width:980px">
          <colgroup>
            <col style="width:31%">
            <col style="width:27%">
            <col style="width:8%">
            ...
            <tr style="border-bottom:1px solid var(--md-outline-variant);background:var(--md-surface-container-low)">
              <th style="padding:.875rem 1.25rem;text-align:left">
                <button type="button" class="sort-btn" onclick="ordenarInadimplentes('cliente')">
                  Nome <span id="sort-cliente" class="md-icon sort-mark"></span>
                </button>
              </th>
              ...
              <th style="padding:.875rem 1.25rem;text-align:center">
                <button type="button" class="sort-btn" style="justify-content:center;width:100%" onclick="ordenarInadimplentes('duplicatas_vencidas')">
                  Qtd. <span id="sort-duplicatas_vencidas" class="md-icon sort-mark"></span>
                </button>
              </th>
              <th style="padding:.875rem 1.25rem;text-align:right">
                <button type="button" class="sort-btn" style="justify-content:flex-end;width:100%" onclick="ordenarInadimplentes('valor_total')">
                  Valor total <span id="sort-valor_total" class="md-icon sort-mark"></span>
                </button>
              </th>
        ...
        <table class="main-table" style="width:100%;border-collapse:collapse;min-width:980px">
          ...
          <thead>
            <tr style="border-bottom:1px solid var(--md-outline-variant);background:var(--md-surface-container-low)">
              ...
              <th style="padding:.875rem 1.25rem;text-align:left;font-size:.7rem;font-weight:600;color:var(--md-on-surface-variant);text-transform:uppercase;letter-spacing:.06em;min-width:170px">Status</th>
              <th style="padding:.875rem 1.25rem;text-align:center;font-size:.7rem;font-weight:600;color:var(--md-on-surface-variant);text-transform:uppercase;letter-spacing:.06em;white-space:nowrap">Qtd.</th>
  ...
  <script>
    ...
    function resumoImportacaoTexto(resumo) {
      ...
      if (resumo.reativados) linhas.push(resumo.reativados + " titulos reativados e confirmados");
      return linhas.join("\\n");
    }
    ...
    function carregarDashboard() {
      ...
      subtituloEl.textContent = "Atualizando dashboard...";
      subtituloEl.removeAttribute("data-tooltip");
      subtituloEl.removeAttribute("title");
      subtituloEl.removeAttribute("tabindex");
      ...
      google.script.run
        ...
        .withFailureHandler(function (erro) {
          subtituloEl.textContent = "Nao foi possivel carregar o dashboard.";
          subtituloEl.removeAttribute("data-tooltip");
          subtituloEl.removeAttribute("title");
          subtituloEl.removeAttribute("tabindex");
          showToast(erro && erro.message ? erro.message : String(erro), "err");
    ...
    function renderDashboard(data) {
      ...
      const subtituloEl = document.getElementById("subtitulo");
      const _detalhesTopo = _totClientes + " inadimplentes · " + _totDups + " duplicatas · " + _atrasoMax + " dias de atraso max.";
      subtituloEl.textContent = _atualEm;
      subtituloEl.setAttribute("data-tooltip", _detalhesTopo);
      subtituloEl.setAttribute("title", _detalhesTopo);
      subtituloEl.setAttribute("tabindex", "0");
    ...
    function buildGrafico_(campo, titulo, subtitulo, defs, counts, totals) {
      ...
      var legendHtml = defs.map(function(d) {
        ...
        var wrapStyle = "display:flex;align-items:flex-start;gap:5px;cursor:pointer;padding:3px 7px;border-radius:999px;border:1px solid transparent;transition:background .15s,opacity .15s,border-color .15s"
          + (ativo ? ";background:var(--ui-accent-container);border-color:var(--ui-accent-border)" : "")
          + (dim ? ";opacity:.4" : "");
    ...
    function isAgendamentoVencido(item) {
      const hoje = new Date().toISOString().substring(0, 10);
      return item && item.status_contato === "AGENDADO" &&
        item.data_agendamento_cliente && item.data_agendamento_cliente < hoje;
    }

    function acoesClienteHtml(item) {
      return "<span class='action-stack'>" +
        "<button type='button' class='secondary small icon-action' title='Copiar resumo' aria-label='Copiar resumo' onclick='copiarResumoCliente(&quot;" + escapeAttr(item.id) + "&quot;, event)'><span class='md-icon'>content_copy</span></button>" +
        "<button type='button' class='secondary small icon-action' title='Atualizar status' aria-label='Atualizar status' onclick='abrirStatusCliente(&quot;" + escapeAttr(item.id) + "&quot;, event)'><span class='md-icon'>edit_note</span></button>" +
      "</span>";
    }
    ...
    function renderInadimplentes(lista) {
      ...
      lista.forEach(function (item) {
        ...
        const _agVencido = isAgendamentoVencido(item);
        tr.className = "cliente-row" + (_agVencido ? " row-ag-vencido" : "");
        tr.dataset.id = item.id;
        tr.innerHTML =
          "<td><div class='cell-nome'>" + clienteNomeHtml(item) + clienteDocumentoHtml(item) + "</div></td>" +
          "<td class='status-cell'>" + statusClienteHtml(item) + "</td>" +
          "<td class='num-center'>" + item.duplicatas_vencidas + "</td>" +
          "<td class='num'><strong>" + moeda.format(item.valor_total || 0) + "</strong></td>" +
          "<td class='num'>" + (item.maior_atraso || 0) + " dias</td>" +
          "<td class='actions'>" + acoesClienteHtml(item) + "</td>";
    ...
    function clienteNomeHtml(item) {
      return "<span class='client-name'><strong>" + escapeHtml(item.cliente) + "</strong></span>";
    }

    function clienteDocumentoHtml(item) {
      ...
      const tag = item.status_cadastro
        ...
        : "";
      return "<span class='cnpj-line'>" + escapeHtml(formatDocumento(item.cnpj_cpf)) + tag + "</span>";
    }
    ...
    function statusClienteHtml(item) {
      ...
      const agVencido = isAgendamentoVencido(item);
      const badgeClass = agVencido ? "status-badge status-scheduled-overdue" : statusContatoClass_(status);
      const statusLabel = agVencido ? "AG. VENCIDO" : status;
      const statusHtml = status
        ? "<span class='" + badgeClass + "'>" + escapeHtml(statusLabel) + "</span>"
        : "<span class='muted'>—</span>";
      const dateHtml = (status === "AGENDADO" && dataAgendamento)
        ? "<span class='status-date'>" + formatDateBR(dataAgendamento) + "</span>"
        : "";
      ...
      const cadDateHtml = ((cadastro === "PROTESTADO" || cadastro === "DESCONSIDERADO") && dataCadastro)
        ...
        : "";
      const obsHtml = observacao ? "<span class='status-note status-observation' title='" + escapeAttr(observacao) + "'>" + escapeHtml(observacao) + "</span>" : "";
      return "<span class='status-main'>" + statusHtml + dateHtml + "</span>" + cadDateHtml + obsHtml;
    }
    ...
    function atualizarMarcadoresOrdenacao() {
      ["cliente", "duplicatas_vencidas", "valor_total"].forEach(function (key) {
        ...
        el.textContent = active ? (sortState.dir === "asc" ? "arrow_drop_up" : "arrow_drop_down") : "";
    ...
    function toggleDetalheCliente(item) {
      ...
      Array.prototype.forEach.call(tb.querySelectorAll("tr.cliente-row.is-open"), function (row) {
        row.classList.remove("is-open");
      });
      target.hidden = !willOpen;
      const mainRow = tb.querySelector('tr.cliente-row[data-id="' + cssEscape(item.id) + '"]');
      if (mainRow && willOpen) mainRow.classList.add("is-open");
    }

    function detalheClienteHtml(item) {
      const dups = item.duplicatas || [];
      if (!dups.length && item.detalhe_indisponivel) {
        return "<div class='inline-detail'>" +
          "<div style='padding:1rem 1.25rem;color:var(--md-on-surface-variant);font-size:.875rem;line-height:1.45'>" +
            "<strong style='color:var(--md-on-surface)'>Detalhamento indisponivel.</strong><br>" +
            "A lista de clientes veio da view do Supabase, mas os titulos da tabela cobranca_titulos nao foram retornados para o GAS. Libere SELECT na tabela/politica RLS ou use a service_role key nas propriedades do Apps Script." +
          "</div>" +
        "</div>";
      }
    ...
    function copiarResumoCliente(id, event) {
      ...
      const texto = [
        ...
        "Documento: " + formatDocumento(item.cnpj_cpf),
    ...
    function abrirStatusCliente(id, event) {
      ...
      document.getElementById("status-cliente").textContent = (item.cliente || "") + " - " + formatDocumento(item.cnpj_cpf || "");
    ...
    function atualizarLinhaClienteOtimista(item) {
      ...
      if (!tr) return;
      tr.className = "cliente-row" + (isAgendamentoVencido(item) ? " row-ag-vencido" : "");
      const tds = tr.querySelectorAll('td');
      if (tds[0]) tds[0].innerHTML = "<div class='cell-nome'>" + clienteNomeHtml(item) + clienteDocumentoHtml(item) + "</div>";
...
function importarArquivoExcelUpload(payload) {
  ...
  try {
    if (!isSupabaseConfigurado_()) setupSistemaCobrancas();
...
function atualizarStatusCliente(payload) {
  ...
  try {
    ...
    const existentes = supabaseGet_(SUPABASE_TABLES.CLIENTES, {
      select: "id,cnpj_cpf,cliente,status_cobranca,status_cadastro,data_agendamento,data_status_cadastro,responsavel,observacao,data_primeira_importacao,origem_arquivo,ativo_na_ultima_importacao",
      cnpj_cpf: "eq." + documento,
      limit: "1"
    }) || [];
    ...
    const merged = {
      ...
      data_status_cadastro: dataStatusCadastro || null,
      tipo_status: "CONTATO",
    ...
    const salvo = supabaseUpsert_(SUPABASE_TABLES.CLIENTES, [merged], "cnpj_cpf", "resolution=merge-duplicates,return=representation");
    const clienteSalvo = Array.isArray(salvo) && salvo.length ? salvo[0] : merged;

    supabasePost_(SUPABASE_TABLES.HISTORICO_STATUS, [{
      cliente_id: clienteSalvo.id || atual.id || null,
      cnpj_cpf: documento,
      cliente: merged.cliente,
      status_cobranca_anterior: atual.status_cobranca || "",
      status_cobranca_novo: status,
      status_cadastro_anterior: atual.status_cadastro || null,
      status_cadastro_novo: statusCadastro || null,
      data_agendamento_anterior: atual.data_agendamento || null,
      data_agendamento_nova: dataAgendamento || null,
      observacao_anterior: atual.observacao || "",
...
function importarCobrancasDeValues_(values, abaOrigem, origemArquivo) {
  if (isSupabaseConfigurado_()) {
    return importarCobrancasDeValuesSupabase_(values, abaOrigem, origemArquivo);
  }
  return importarCobrancasDeValuesSheets_(values, abaOrigem, origemArquivo);
}

function importarCobrancasDeValuesSupabase_(values, abaOrigem, origemArquivo) {
  ...
  const resumo = {
    ...
    removidos: 0,
    observacao: "Importacao Supabase"
  };

  const existentesRows = supabaseGet_(SUPABASE_TABLES.TITULOS, {
    select: BASE_HEADERS.concat(["cliente_id"]).join(","),
    limit: "10000"
  }) || [];
  const base = montarBaseSupabase_(existentesRows);
  const hashesImportadas = {};
  const titulosUpsert = [];
  const duplicidadesInsert = [];
  ...
  importacao.registros.forEach(function (registro) {
    ...
    if (existente) {
      ...
      if (existente.obj.hash_conteudo !== registro.hash_conteudo ||
          tituloSupabaseDivergeDoRegistro_(existente.obj, registro)) {
        resumo.atualizados += 1;
      }
      // Mesmo com hash de conteudo igual, regrava os campos da origem. Isso
      // corrige bases migradas onde hashes estavam certos, mas colunas antigas
      // como CNPJ/CPF, numero ou vencimento ficaram divergentes.
      titulosUpsert.push(montarTituloSupabaseAtualizado_(existente.obj, registro, origemArquivo, nowText));
      return;
    }
    ...
    if (parecidos.length) {
      parecidos.forEach(function (item) {
        duplicidadesInsert.push(montarDuplicidadeSupabase_(item, registro, nowText));
      });
    ...
    registro.ativo_na_ultima_importacao = true;

    titulosUpsert.push(tituloSupabasePayload_(registro));
    base.lista.push({ row: null, obj: Object.assign({}, registro) });
    base.porHash[registro.hash_identificacao] = { row: null, obj: Object.assign({}, registro) };
    resumo.novas_cobrancas += 1;
  });

  const titulosInativar = [];
  if (importacao.registros.length) {
    base.lista.forEach(function (item) {
      const hash = item.obj.hash_identificacao;
      const ativo = item.obj.ativo_na_ultima_importacao;
      if (hash && !hashesImportadas[hash] && ativo !== false && String(ativo).toLowerCase() !== "false") {
        titulosInativar.push(hash);
      }
    });
  }

  sincronizarClientesSupabase_(importacao.registros, origemArquivo, nowText);
  const clientesPorDocumento = carregarClientesSupabasePorDocumento_();
  titulosUpsert.forEach(function (titulo) {
    const cliente = clientesPorDocumento[titulo.cnpj_cpf];
    if (cliente && cliente.id) titulo.cliente_id = cliente.id;
  });

  if (titulosUpsert.length) {
    supabaseUpsert_(SUPABASE_TABLES.TITULOS, titulosUpsert, "hash_identificacao", "resolution=merge-duplicates,return=minimal");
  }

  // Uma importacao representa o retrato completo das cobrancas em aberto.
  // Confirma no banco que cada hash lida ficou ativa antes de inativar titulos ausentes.
  const reconciliacaoTitulos = garantirTitulosImportadosAtivosSupabase_(
    Object.keys(hashesImportadas),
    origemArquivo,
    nowText
  );

  titulosInativar.forEach(function (hash) {
    supabasePatch_(SUPABASE_TABLES.TITULOS, {
      ativo_na_ultima_importacao: false,
      data_ultima_importacao: nowText,
      origem_arquivo: origemArquivo
    }, {
      hash_identificacao: "eq." + hash
    }, "return=minimal");
  });
  resumo.removidos = titulosInativar.length;
  resumo.reativados = reconciliacaoTitulos.reativados;

  if (duplicidadesInsert.length) {
    supabasePost_(SUPABASE_TABLES.DUPLICIDADES, duplicidadesInsert, null, "return=minimal");
  }

  supabasePost_(SUPABASE_TABLES.LOGS, [{
    data_importacao: nowText,
    versao: APP_VERSION,
    origem_arquivo: origemArquivo,
    aba_origem: abaOrigem,
...
function garantirTitulosImportadosAtivosSupabase_(hashes, origemArquivo, nowText) {
  const listaHashes = (hashes || []).filter(Boolean);
  if (!listaHashes.length) return { confirmados: 0, reativados: 0 };

  const encontrados = {};
  let reativados = 0;

  // Hashes SHA-256 deixam filtros `in.(...)` longos. 15 por lote fica abaixo
  // do limite de URL do UrlFetchApp com margem para os demais parametros.
  for (let inicio = 0; inicio < listaHashes.length; inicio += SUPABASE_HASH_RECONCILIATION_BATCH_SIZE) {
    const lote = listaHashes.slice(inicio, inicio + SUPABASE_HASH_RECONCILIATION_BATCH_SIZE);
    const filtroHash = "in.(" + lote.join(",") + ")";
    const antes = supabaseGet_(SUPABASE_TABLES.TITULOS, {
      select: "hash_identificacao,ativo_na_ultima_importacao",
      hash_identificacao: filtroHash,
      limit: String(SUPABASE_HASH_RECONCILIATION_BATCH_SIZE)
    }) || [];

    reativados += antes.filter(function (titulo) {
      return titulo.ativo_na_ultima_importacao === false ||
        String(titulo.ativo_na_ultima_importacao).toLowerCase() === "false";
    }).length;
    ...
    const depois = supabaseGet_(SUPABASE_TABLES.TITULOS, {
      select: "hash_identificacao,ativo_na_ultima_importacao",
      hash_identificacao: filtroHash,
      limit: String(SUPABASE_HASH_RECONCILIATION_BATCH_SIZE)
    }) || [];

    depois.forEach(function (titulo) {
      const hash = titulo.hash_identificacao || "";
      const ativo = titulo.ativo_na_ultima_importacao;
      if (hash && ativo !== false && String(ativo).toLowerCase() !== "false") {
        encontrados[hash] = true;
      }
    });
  }

  const ausentes = listaHashes.filter(function (hash) {
    return !encontrados[hash];
  });

  if (ausentes.length) {
    throw new Error(
      "Importacao interrompida: " + ausentes.length +
      " titulo(s) lido(s) do Excel nao foram confirmados como ativos no Supabase. " +
      "Nenhuma importacao deve ser considerada concluida sem essa confirmacao."
    );
  }

  return { confirmados: listaHashes.length, reativados: reativados };
}

function montarBaseSupabase_(rows) {
  const lista = [];
  const porHash = {};

  (rows || []).forEach(function (row) {
    const obj = normalizarCobrancaBase_(row);
    const item = { row: null, obj: obj };
    lista.push(item);
    if (obj.hash_identificacao) porHash[obj.hash_identificacao] = item;
  });

  return { lista: lista, porHash: porHash };
}

function montarTituloSupabaseAtualizado_(atual, registro, origemArquivo, nowText) {
  const merged = Object.assign({}, atual);
  ...
  merged.hash_identificacao = registro.hash_identificacao;
  merged.data_ultima_importacao = nowText;
  merged.origem_arquivo = origemArquivo;
  merged.ativo_na_ultima_importacao = true;

  MANUAL_FIELDS.forEach(function (field) {
    merged[field] = atual[field] || "";
  });

  return tituloSupabasePayload_(merged);
}

function tituloSupabaseDivergeDoRegistro_(atual, registro) {
  return HASH_CONTEUDO_FIELDS.some(function (field) {
    return String(atual[field] || "") !== String(registro[field] || "");
  });
}

function montarTituloSupabaseTecnico_(atual, origemArquivo, nowText) {
  const merged = Object.assign({}, atual);
  merged.data_ultima_importacao = nowText;
  merged.origem_arquivo = origemArquivo;
  merged.ativo_na_ultima_importacao = true;
...
function tituloSupabasePayload_(obj) {
  return {
    id_interno: obj.id_interno || "",
    hash_identificacao: obj.hash_identificacao || "",
    hash_conteudo: obj.hash_conteudo || "",
    data_primeira_importacao: obj.data_primeira_importacao || null,
    data_ultima_importacao: obj.data_ultima_importacao || null,
    origem_arquivo: obj.origem_arquivo || "",
    cliente_id: obj.cliente_id || null,
    ativo_na_ultima_importacao: obj.ativo_na_ultima_importacao === false ? false : String(obj.ativo_na_ultima_importacao || "true").toUpperCase() !== "NAO",
    cliente: obj.cliente || "",
    cnpj_cpf: normalizeDocumento_(obj.cnpj_cpf),
    emissao: obj.emissao || null,
    vencimento: obj.vencimento || null,
    categoria: obj.categoria || "",
    parcela: obj.parcela || "",
    numero: obj.numero || "",
    boleto: obj.boleto || "",
    valor_conta: parseNumber_(obj.valor_conta),
    valor_recebido: parseNumber_(obj.valor_recebido),
    valor_a_receber: parseNumber_(obj.valor_a_receber),
    status_cobranca: obj.status_cobranca || "",
    observacao: obj.observacao || "",
    responsavel: obj.responsavel || "",
    status_negociacao: obj.status_negociacao || ""
  };
}

function montarDuplicidadeSupabase_(existente, registro, nowText) {
  return {
    data_deteccao: nowText,
    tipo: "POSSIVEL_DUPLICIDADE",
    id_interno_existente: existente.obj.id_interno || "",
    hash_existente: existente.obj.hash_identificacao || "",
    hash_importada: registro.hash_identificacao || "",
    cliente: registro.cliente || "",
    cnpj_cpf: registro.cnpj_cpf || "",
    emissao: registro.emissao || null,
    vencimento: registro.vencimento || null,
    categoria: registro.categoria || "",
    parcela: registro.parcela || "",
    numero: registro.numero || "",
    valor_conta: parseNumber_(registro.valor_conta),
    observacao_tecnica: "Mesmo documento e vencimento, com identificadores semelhantes. Revisar antes de unir ou descartar."
  };
}

function sincronizarClientesSupabase_(registros, origemArquivo, nowText) {
  const porDocumento = carregarClientesSupabasePorDocumento_();

  const vistos = {};
  const payloadsPorDocumento = {};
  ...
  registros.forEach(function (registro) {
    ...
    const atual = porDocumento[documento] || {};

    payloadsPorDocumento[documento] = {
      cnpj_cpf: documento,
      cliente: registro.cliente || atual.cliente || "",
      status_cobranca: normalizarStatusCliente_(atual.status_cobranca),
      status_cadastro: nullableStatusCadastro_(atual.status_cadastro),
      data_agendamento: atual.data_agendamento || null,
      data_status_cadastro: atual.data_status_cadastro || null,
      tipo_status: "CONTATO",
      responsavel: atual.responsavel || "",
  ...
  const payloads = Object.keys(payloadsPorDocumento).map(function (documento) {
    return payloadsPorDocumento[documento];
  });

  if (payloads.length) {
    supabaseUpsert_(SUPABASE_TABLES.CLIENTES, payloads, "cnpj_cpf", "resolution=merge-duplicates,return=minimal");
  }

  if (registros.length) {
    Object.keys(porDocumento).forEach(function (documento) {
      const atual = porDocumento[documento];
      if (!vistos[documento] && atual.ativo_na_ultima_importacao !== false && String(atual.ativo_na_ultima_importacao).toLowerCase() !== "false") {
        supabasePatch_(SUPABASE_TABLES.CLIENTES, {
          ativo_na_ultima_importacao: false,
          data_ultima_importacao: nowText,
          origem_arquivo: atual.origem_arquivo || origemArquivo
        }, {
          cnpj_cpf: "eq." + documento
        }, "return=minimal");
      }
...
function carregarClientesSupabasePorDocumento_() {
  const existentesRows = supabaseGet_(SUPABASE_TABLES.CLIENTES, {
    select: "id,cnpj_cpf,cliente,status_cobranca,status_cadastro,data_agendamento,data_status_cadastro,tipo_status,responsavel,observacao,data_primeira_importacao,data_ultima_importacao,origem_arquivo,ativo_na_ultima_importacao",
    limit: "10000"
  }) || [];
  const porDocumento = {};
  existentesRows.forEach(function (row) {
    if (row.cnpj_cpf) porDocumento[normalizeDocumento_(row.cnpj_cpf)] = row;
  });
  return porDocumento;
}

function importarCobrancasDeValuesSheets_(values, abaOrigem, origemArquivo) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const baseSheet = ss.getSheetByName(SHEETS.BASE);
...
function nullableStatusCadastro_(status) {
  return normalizarStatusCadastro_(status) || null;
}
