...
<head>
  ...
  <style type="text/tailwindcss">
    @layer components {
      ...
      .detail-tbl tbody tr:hover { background: #eef2ff; }
      .detail-tbl tbody td {
        padding: .625rem 1rem;
        font-size: .8125rem;
        color: #374151;
      ...
      .detail-tbl tfoot tr { border-top: 2px solid #e5e7eb; }
      .detail-tbl tfoot td {
        padding: .5rem 1rem;
        font-size: .8125rem;
        background: #f8fafc;
      ...
      .status-finalizado { @apply bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-200; }
...
<body class="bg-gray-50 text-gray-900 min-h-screen" style="font-family:ui-sans-serif,system-ui,-apple-system,'Segoe UI',sans-serif;-webkit-font-smoothing:antialiased">
  ...
  <main style="max-width:1280px;margin:0 auto;padding:2rem 1.5rem 3rem">
    ...
    <header class="flex items-center justify-between mb-8">
      <div>
        ...
        <p id="subtitulo" class="muted" style="font-size:.8125rem;margin:.25rem 0 0">Carregando dados da base...</p>
      </div>
      <div style="display:flex;align-items:center;gap:.5rem;position:relative">
        <div style="position:relative">
          <button id="btn-import-trigger" type="button" onclick="toggleImportMenu()"
            style="display:inline-flex;align-items:center;gap:.375rem;height:32px;padding:0 .75rem;font-size:.8125rem;font-weight:500;color:#374151;background:#fff;border:1px solid #e5e7eb;border-radius:.5rem;cursor:pointer;transition:background .15s"
            onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background='#fff'">
            <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/>
            </svg>
            Importar
            <svg width="11" height="11" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" style="opacity:.5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/>
            </svg>
          </button>
          <div id="import-menu" style="display:none;position:absolute;right:0;top:calc(100% + .5rem);width:370px;background:#fff;border:1px solid #e5e7eb;border-radius:.75rem;box-shadow:0 8px 24px rgba(0,0,0,.1);padding:1rem 1.25rem;z-index:100">
            <p style="font-size:.7rem;font-weight:600;color:#9ca3af;text-transform:uppercase;letter-spacing:.08em;margin:0 0 .875rem">Importação</p>
            <div style="display:flex;flex-direction:column;gap:.75rem">
              <div>
                <label for="arquivo-excel" style="display:block;font-size:.75rem;font-weight:500;color:#374151;margin-bottom:.375rem">Arquivo Excel (.xlsx)</label>
                <input id="arquivo-excel" type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                  style="display:block;width:100%;font-size:.8125rem;color:#6b7280;cursor:pointer"
              ...
              <div style="display:flex;gap:.5rem;flex-wrap:wrap">
                ...
                <button type="button" onclick="importarAbaPadrao()" class="btn-secondary">Importar aba</button>
    ...
    <div style="display:grid;grid-template-columns:0.6fr 0.6fr 1fr 1fr;gap:1rem;margin-bottom:1.5rem">
      <div style="background:#fff;border:1px solid #f3f4f6;border-radius:.75rem;box-shadow:0 1px 3px rgba(0,0,0,.06);padding:1.25rem">
        ...
        <strong id="kpi-vencido" style="display:block;font-size:1.5rem;font-weight:700;color:#ef4444;margin-top:.25rem;font-variant-numeric:tabular-nums;letter-spacing:-.025em">R$ 0,00</strong>
      </div>
      <div style="background:#fff;border:1px solid #f3f4f6;border-radius:.75rem;box-shadow:0 1px 3px rgba(0,0,0,.06);padding:1.25rem">
        <span style="display:block;font-size:.7rem;font-weight:600;color:#9ca3af;text-transform:uppercase;letter-spacing:.08em">Total protestado</span>
        <strong id="kpi-protestado" style="display:block;font-size:1.5rem;font-weight:700;color:#7c3aed;margin-top:.25rem;font-variant-numeric:tabular-nums;letter-spacing:-.025em">R$ 0,00</strong>
      </div>
      <div id="grafico-contato" style="background:#fff;border:1px solid #f3f4f6;border-radius:.75rem;box-shadow:0 1px 3px rgba(0,0,0,.06);padding:1.25rem;min-height:90px"></div>
      <div id="grafico-cadastro" style="background:#fff;border:1px solid #f3f4f6;border-radius:.75rem;box-shadow:0 1px 3px rgba(0,0,0,.06);padding:1.25rem;min-height:90px"></div>
    </div>
    ...
    <div style="background:#fff;border:1px solid #f3f4f6;border-radius:.75rem;box-shadow:0 1px 3px rgba(0,0,0,.06);overflow:hidden">
      ...
      <div style="padding:.625rem 1.25rem;border-bottom:1px solid #f3f4f6">
        <div style="position:relative;display:flex;align-items:center">
          <svg width="15" height="15" viewBox="0 0 20 20" fill="none" stroke="#9ca3af" stroke-width="2" style="position:absolute;left:.625rem;flex-shrink:0;pointer-events:none">
            <circle cx="8.5" cy="8.5" r="5.5"/><path stroke-linecap="round" d="m13.5 13.5 3 3"/>
          </svg>
          <input id="filtro-cliente" type="text" placeholder="Filtrar por nome..." autocomplete="off"
            ...
            style="width:100%;height:34px;padding:0 2rem 0 2.125rem;font-size:.8125rem;border:1px solid #e5e7eb;border-radius:.5rem;outline:none;color:#111827;background:#fff;transition:border-color .15s"
            onfocus="this.style.borderColor='#6366f1'" onblur="this.style.borderColor='#e5e7eb'">
          <button id="filtro-clear" type="button" onclick="limparFiltro()"
            style="display:none;position:absolute;right:.375rem;width:22px;height:22px;align-items:center;justify-content:center;border:none;background:transparent;cursor:pointer;border-radius:4px;color:#9ca3af;padding:0;transition:background .1s,color .1s"
    ...
    <!-- ── Finalizados ── -->
    <div id="finalizados-section" style="display:none;margin-top:1.5rem;background:#fff;border:1px solid #f3f4f6;border-radius:.75rem;box-shadow:0 1px 3px rgba(0,0,0,.06);overflow:hidden">
      <div style="display:flex;align-items:center;justify-content:space-between;padding:.875rem 1.25rem;border-bottom:1px solid #f3f4f6">
        <div>
          <h2 style="font-size:1rem;font-weight:600;color:#111827;margin:0">Finalizados</h2>
        ...
        <table style="width:100%;border-collapse:collapse;min-width:700px">
          <thead>
            <tr style="border-bottom:1px solid #f3f4f6">
              <th style="padding:.75rem 1.25rem;text-align:left;font-size:.7rem;font-weight:600;color:#9ca3af;text-transform:uppercase;letter-spacing:.08em">Cliente</th>
              <th style="padding:.75rem 1.25rem;text-align:left;font-size:.7rem;font-weight:600;color:#9ca3af;text-transform:uppercase;letter-spacing:.08em">Status</th>
              <th style="padding:.75rem 1.25rem;text-align:right;font-size:.7rem;font-weight:600;color:#9ca3af;text-transform:uppercase;letter-spacing:.08em;white-space:nowrap">Duplicatas</th>
              <th style="padding:.75rem 1.25rem;text-align:right;font-size:.7rem;font-weight:600;color:#9ca3af;text-transform:uppercase;letter-spacing:.08em">Total</th>
              <th style="padding:.75rem 1.25rem;text-align:right;font-size:.7rem;font-weight:600;color:#9ca3af;text-transform:uppercase;letter-spacing:.08em;white-space:nowrap">Maior atraso</th>
              <th class="actions" style="padding:.75rem 1.25rem;font-size:.7rem;font-weight:600;color:#9ca3af;text-transform:uppercase;letter-spacing:.08em">Ações</th>
            </tr>
          </thead>
  ...
  <script>
    ...
    function renderDashboard(data) {
      ...
      inadimplentesView = _todos.filter(function(i) { return (i.duplicatas_vencidas || 0) > 1 && i.status_contato !== "FINALIZADO"; }).slice();
    ...
    function toggleImportMenu() {
      ...
      if (!isOpen) {
        setTimeout(function() {
          document.addEventListener("click", _closeImportOnOutside, true);
        }, 0);
      }
    }
    ...
    function _closeImportOnOutside(e) {
      ...
      if (menu && !menu.contains(e.target) && trigger && !trigger.contains(e.target)) {
        ...
        document.removeEventListener("click", _closeImportOnOutside, true);
      }
    }

    function renderGraficos(inadimplentes) {
      var filtrado = (inadimplentes || []).filter(function(i) { return (i.duplicatas_vencidas || 0) > 1 && i.status_contato !== "FINALIZADO"; });
      ...
      var contatoCounts = {};
      CONTATO_DEFS.forEach(function(d) { contatoCounts[d.key] = 0; });
      ...
      var cadastroCounts = {};
      CADASTRO_DEFS.forEach(function(d) { cadastroCounts[d.key] = 0; });
      filtrado.forEach(function(i) {
      ...
      if (el1) el1.innerHTML = buildGrafico_("STATUS DE CONTATO", total + " cliente(s) c/ 2+ duplicatas", CONTATO_DEFS, contatoCounts);
      if (el2) el2.innerHTML = buildGrafico_("STATUS DE CADASTRO", totalCadastro + " com status especial (de " + total + ")", CADASTRO_DEFS, cadastroCounts);
    }

    function buildGrafico_(titulo, subtitulo, defs, counts) {
      ...
      var barHtml = totalVis
        ? defs.map(function(d) {
            ...
            return "<div title='" + escapeHtml(d.key) + ": " + n + "' style='flex:" + pct + ";background:" + d.cor + ";min-width:2px'></div>";
          }).join("")
      ...
      var legendHtml = defs.map(function(d) {
        ...
        return "<div style='display:flex;align-items:center;gap:5px'>" +
          "<span style='width:9px;height:9px;border-radius:50%;background:" + d.cor + ";flex-shrink:0;display:inline-block'></span>" +
          "<span style='font-size:.7rem;color:" + d.textCor + ";white-space:nowrap'>" + escapeHtml(d.key) + " <strong style='color:#111827'>" + n + "</strong></span>" +
        "</div>";
      }).join("");
      if (!legendHtml) legendHtml = "<span style='font-size:.7rem;color:#9ca3af'>Nenhum</span>";

      return "<span style='display:block;font-size:.7rem;font-weight:600;color:#9ca3af;text-transform:uppercase;letter-spacing:.08em'>" + titulo + "</span>" +
        "<span style='display:block;font-size:.75rem;color:#6b7280;margin:.2rem 0 0'>" + subtitulo + "</span>" +
        "<div style='display:flex;height:8px;border-radius:4px;overflow:hidden;margin:.875rem 0 .75rem;background:#f3f4f6'>" + barHtml + "</div>" +
        "<div style='display:flex;flex-wrap:wrap;gap:5px 14px'>" + legendHtml + "</div>";
    }
    ...
    function filtrarInadimplentes(termo) {
      ...
      const lista = q
        ? (inadimplentesView || []).filter(function (i) {
    ...
    function renderInadimplentes(lista) {
      ...
      body.innerHTML = "";
      document.getElementById("lista-info").textContent = lista.length + " cliente(s) com 2+ duplicatas vencidas.";
    ...
    function renderFinalizados(lista) {
      ...
      if (info) info.textContent = lista.length + " cliente(s) finalizado(s).";
      body.innerHTML = "";
      lista.forEach(function(item) {
        const tr = document.createElement("tr");
        tr.className = "cliente-row";
        tr.dataset.id = item.id;
        tr.innerHTML =
          "<td><div class='cell-nome'><span class='client-name'><strong>" + escapeHtml(item.cliente) + "</strong></span><span class='cnpj-line'>" + escapeHtml(item.cnpj_cpf) + "</span></div></td>" +
          "<td class='status-cell'>" + statusClienteHtml(item) + "</td>" +
          "<td class='num'>" + item.duplicatas_vencidas + "</td>" +
          "<td class='num'><strong>" + moeda.format(item.valor_total || 0) + "</strong></td>" +
          "<td class='num'>" + (item.maior_atraso || 0) + " dias</td>" +
