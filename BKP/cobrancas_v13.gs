// SISTEMA DE GESTAO DE COBRANCAS - Apps Script v13
...
function aplicarCadastroClienteAoGrupo_(grupo, clienteCadastro) {
  const status = clienteCadastro ? normalizarStatusCliente_(clienteCadastro.status_cobranca) : STATUS_CLIENTE_PADRAO;
  const tipo = statusClienteTipo_(status);

  grupo.status_cliente = status;
  grupo.tipo_status_cliente = tipo;
  grupo.status_contato = tipo === "CONTATO" ? status : "";
  grupo.status_cadastro = tipo === "CADASTRO" ? status : "";
  ...
  <style>
    ...
    * {
    ...
    .modal {
      ...
      border: 1px solid var(--line);
      border-radius: 8px;
      box-shadow: 0 18px 44px rgba(23, 32, 51, 0.24);
      padding: 18px;
    }
...
<body>
    ...
    <div class="modal">
      <div class="modal-head">
        <div>
          ...
          <div id="status-cliente" class="muted"></div>
        </div>
      ...
      <div class="form-grid">
        ...
        <label for="status-observacao">Observacao</label>
        <textarea id="status-observacao"></textarea>
        <div class="toolbar-actions">
          <button type="button" onclick="salvarStatusCliente()">Salvar</button>
          <button type="button" class="secondary" onclick="fecharStatusModal()">Cancelar</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    function renderInadimplentes(lista) {
      ...
      if (!lista.length) {
        body.innerHTML = '<tr><td colspan="6" class="muted">Nenhum inadimplente encontrado.</td></tr>';
        return;
      }
      ...
      lista.forEach(function (item) {
        ...
        tr.innerHTML =
          "<td>" + clienteNomeHtml(item) + "<br><span class='muted'>" + escapeHtml(item.cnpj_cpf) + "</span></td>" +
          ...
          "<td class='actions'><span class='action-stack'>" +
            "<button type='button' class='secondary small' onclick='copiarResumoCliente(&quot;" + escapeAttr(item.id) + "&quot;, event)'>Copiar resumo</button>" +
    ...
    function clienteNomeHtml(item) {
      const tag = item.status_cadastro ? " <span class='cadastro-tag'>" + escapeHtml(item.status_cadastro) + "</span>" : "";
    ...
    function statusClienteHtml(item) {
      ...
      const statusHtml = status ? "<span class='status-main'>" + escapeHtml(status) + "</span>" : "<span class='muted'>-</span>";
      const obsHtml = observacao ? "<span class='status-note'>" + escapeHtml(observacao) + "</span>" : "";
      return statusHtml + obsHtml;
    }
    ...
    function abrirStatusCliente(id, event) {
      ...
      document.getElementById("status-select").value = item.status_cliente || "EM ABERTO";
    ...
    function salvarStatusCliente() {
      ...
      const payload = {
        ...
        cliente: item.cliente || "",
        status_cobranca: document.getElementById("status-select").value,
