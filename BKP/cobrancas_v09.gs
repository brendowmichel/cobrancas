...
function getDashboardCobrancas() {
  ...
  base.forEach(function (cobranca) {
    ...
    } else if (vencimento && diasEntre_(hoje, vencimento) <= 7) {
      qtdVence7 += 1;
      proximos.push(montarResumoCobranca_(cobranca, valorAberto, 0));
    }
  });
  ...
  return {
    ...
    kpis: {
      ...
      vence_7_dias: qtdVence7,
      maior_atraso: maiorAtraso
    },
    inadimplentes: inadimplentes.slice(0, 200),
    proximos_vencimentos: proximos.slice(0, 12),
    sugestoes: montarSugestoesCobranca_(inadimplentes, totalVencido, qtdVence7)
  };
}
...
function montarSugestoesCobranca_(inadimplentes, totalVencido, qtdVence7) {
  const sugestoes = [];

  if (inadimplentes.length) {
    sugestoes.push("Comece pelos clientes com maior valor vencido e maior atraso.");
  }

  if (totalVencido > 0) {
    sugestoes.push("Separe a rotina em blocos: cobrar vencidos acima de 30 dias, depois 8 a 30 dias, depois vencidos recentes.");
  }

  if (qtdVence7 > 0) {
    sugestoes.push("Envie lembretes preventivos para cobranças que vencem nos proximos 7 dias.");
  }

  sugestoes.push("Use observacao, responsavel e status de negociacao para registrar cada contato e evitar retrabalho.");
...
<body>
    ...
    <section class="layout">
      <div class="panel panel-pad">
        <div class="toolbar">
          <div>
            <h2>Inadimplentes</h2>
            <div class="muted" id="lista-info">Nome, duplicatas vencidas e valor total em aberto.</div>
          </div>
          <div class="toolbar-actions">
        ...
        <div class="table-wrap">
          <table>
            <thead>
              <tr>
                <th>Nome</th>
                <th class="num">Duplicatas vencidas</th>
      ...
      <aside>
        <div class="panel panel-pad" id="detalhe-cliente">
          <h2>Detalhamento</h2>
          <p>Selecione um cliente inadimplente para ver as duplicatas vencidas.</p>
        </div>
        ...
        <div class="panel panel-pad upcoming">
          <h2>Vencem em ate 7 dias</h2>
          <div id="proximos"></div>
        </div>
      </aside>
    </section>
  ...
  <script>
    ...
    function renderDashboard(data) {
      ...
      renderInadimplentes(data.inadimplentes || []);
      renderSugestoes(data.sugestoes || []);
      renderProximos(data.proximos_vencimentos || []);
    }
    ...
    function renderInadimplentes(lista) {
      ...
      if (!lista.length) {
        body.innerHTML = '<tr><td colspan="4" class="muted">Nenhum inadimplente encontrado.</td></tr>';
        document.getElementById("detalhe-cliente").innerHTML = "<h2>Detalhamento</h2><p>Nenhum cliente inadimplente no momento.</p>";
        return;
      }
      ...
      lista.forEach(function (item) {
        ...
        tr.addEventListener("click", function () {
          renderDetalheCliente(item);
        });
        body.appendChild(tr);
      });

      renderDetalheCliente(lista[0]);
    }

    function renderDetalheCliente(item) {
      const el = document.getElementById("detalhe-cliente");
      const duplicatas = item.duplicatas || [];
      const linhas = duplicatas.map(function (dup) {
        return "<div class='duplicata'>" +
          "<div><strong>" + escapeHtml(dup.numero || "Sem numero") + "</strong> <span class='pill'>" + escapeHtml(dup.status_cobranca || "VENCIDO") + "</span></div>" +
          "<div class='duplicata-row'><span>Vencimento</span><strong>" + escapeHtml(dup.vencimento) + " (" + dup.dias_atraso + " dias)</strong></div>" +
          "<div class='duplicata-row'><span>Categoria</span><strong>" + escapeHtml(dup.categoria) + "</strong></div>" +
          "<div class='duplicata-row'><span>Parcela</span><strong>" + escapeHtml(dup.parcela) + "</strong></div>" +
          "<div class='duplicata-row'><span>Boleto</span><strong>" + escapeHtml(dup.boleto) + "</strong></div>" +
          "<div class='duplicata-row'><span>A receber</span><strong>" + moeda.format(dup.valor_a_receber || 0) + "</strong></div>" +
          "</div>";
      }).join("");

      el.innerHTML =
        "<div class='detail-head'>" +
          "<h2>" + escapeHtml(item.cliente) + "</h2>" +
          "<span class='muted'>" + escapeHtml(item.cnpj_cpf) + "</span>" +
          "<strong>" + item.duplicatas_vencidas + " duplicata(s) vencida(s) - " + moeda.format(item.valor_total || 0) + "</strong>" +
        "</div>" +
        "<button type='button' class='secondary small' onclick='copiarResumoCliente(&quot;" + escapeAttr(item.id) + "&quot;)'>Copiar resumo</button>" +
    ...
    function renderSugestoes(lista) {
      ...
      el.innerHTML = lista.map(function (item) {
        return "<div class='muted'>" + escapeHtml(item) + "</div>";
      }).join("");
    }
    ...
    function renderProximos(lista) {
      ...
      if (!lista.length) {
        el.innerHTML = "<div class='muted'>Nenhuma cobranca vencendo nos proximos 7 dias.</div>";
        return;
      }
      ...
      el.innerHTML = lista.map(function (item) {
        return "<div class='duplicata'>" +
          ...
          "<div class='duplicata-row'><span>Vence</span><strong>" + escapeHtml(item.vencimento) + "</strong></div>" +
          "<div class='duplicata-row'><span>Valor</span><strong>" + moeda.format(item.valor_a_receber || 0) + "</strong></div>" +
        "</div>";
      }).join("");
    }

    function copiarResumoCliente(id) {
      ...
      const texto = [
        ...
        (item.duplicatas || []).map(function (dup) {
          return "- " + (dup.numero || "Sem numero") + " | venc. " + dup.vencimento + " | " + moeda.format(dup.valor_a_receber || 0);
        }).join("\\n")
      ].join("\\n");
