// SISTEMA DE GESTAO DE COBRANCAS - Apps Script v41
...
function gerarRelatorioPdf(tipo) {
  ...
  function linhaDuplicata(dup) {
    return "<tr>" +
      "<td><strong>" + esc(dup.numero || "Sem número") + "</strong>" + ((dup.itens_agrupados || 0) > 1 ? "<br><span class='doc'>" + dup.itens_agrupados + " itens agrupados</span>" : "") + "</td>" +
      "<td>" + esc(dup.parcela || "-") + "</td>" +
      "<td>" + esc(dbr(dup.vencimento)) + "</td>" +
      "<td class='right'>" + Number(dup.dias_atraso || 0) + " dias</td>" +
      "<td>" + esc(dup.categoria || "-") + "</td>" +
      "<td class='right money-col'>" + moeda(dup.valor_conta || 0) + "</td>" +
      "<td class='right money-col'>" + moeda(dup.valor_recebido || 0) + "</td>" +
  ...
  function clienteDetalhado(item, idx, cor) {
    ...
    return "<table class='client-table' style='--section-color:" + cor + "'>" +
      "<thead>" +
        "<tr class='client-head'><th colspan='9'>" +
          "<span class='client-index'>" + idx + "</span>" +
          "<span><strong>" + esc(item.cliente) + "</strong><small>" + esc(documentoFormatado(item.cnpj_cpf)) + "</small></span>" +
          "<span class='client-meta'>Contato: " + esc(contatoTxt(item)) + " &middot; Cadastro: " + esc(cadastroTxt(item)) + " &middot; " + duplicatas.length + " duplicatas &middot; " + moeda(item.valor_total || 0) + "</span>" +
        "</th></tr>" +
        (item.observacao_cliente ? "<tr class='client-note'><th colspan='9'><strong>Observação:</strong> " + esc(item.observacao_cliente) + "</th></tr>" : "") +
        "<tr class='columns'><th>Número/RPS</th><th>Parcela</th><th>Vencimento</th><th class='right'>Atraso</th><th>Categoria</th><th class='right'>Valor</th><th class='right'>Recebido</th><th class='right'>A receber</th><th>Status</th></tr>" +
      "</thead>" +
      "<tbody>" + rows + "</tbody>" +
      "<tfoot><tr><td colspan='7'>" + duplicatas.length + " duplicatas</td><td class='right'><strong>" + moeda(item.valor_total || 0) + "</strong></td><td></td></tr></tfoot>" +
    "</table>";
  }
  ...
  var orientacao = tipo === "detalhado" ? "landscape" : "portrait";

  var css = "@page{size:A4 " + orientacao + ";margin:12mm 10mm 14mm}" +
    ...
    ".page-footer{position:fixed;bottom:-9mm;left:0;right:0;border-top:1px solid #dadce0;padding-top:3px;color:#777;font-size:7.5px;display:flex;justify-content:space-between}" +
    ...
    ".client-table{margin:0 0 12px;border:1px solid #dfe3e8;break-inside:auto}.client-table thead{display:table-header-group}.client-table .client-head th{padding:7px 8px;background:#f4f7fc;border-left:4px solid var(--section-color);text-transform:none;letter-spacing:0;color:#202124}" +
    ".client-head th{display:flex;align-items:center;gap:8px}.client-head strong{display:block;font-size:10px}.client-head small{display:block;font-size:7.5px;color:#6b7280;font-weight:normal}.client-index{width:22px;height:22px;border-radius:50%;background:var(--section-color);color:#fff;display:inline-flex;align-items:center;justify-content:center}.client-meta{margin-left:auto;font-size:8px;font-weight:normal;color:#4b5563;text-align:right}" +
    ".client-note th{background:#fffbe8;color:#5f4b00;text-transform:none;letter-spacing:0;font-weight:normal;padding:5px 8px}.client-table .columns th{font-size:6.8px}.client-table tfoot td{background:#f8f9fa;border-top:1px solid #cfd4dc;border-bottom:0}" +
    ".client-table th:nth-child(1){width:11%}.client-table th:nth-child(2){width:7%}.client-table th:nth-child(3){width:9%}.client-table th:nth-child(4){width:7%}.client-table th:nth-child(5){width:25%}.client-table th:nth-child(6),.client-table th:nth-child(7),.client-table th:nth-child(8){width:10%}.client-table th:nth-child(9){width:11%}";
  ...
  var html = "<!DOCTYPE html><html><head><meta charset='utf-8'><style>" + css + "</style></head><body>" +
    ...
    "<div class='page-footer'><span>Sistema de Gestão de Cobranças</span><span class='page-number'></span></div>" +
    conteudo +
    "</body></html>";
