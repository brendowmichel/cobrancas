// SISTEMA DE GESTAO DE COBRANCAS - Apps Script v39
...
<head>
  ...
  <style type="text/tailwindcss">
    ...
    @layer components {
      ...
      /* === Cards M3 ========================================= */
      .md-card { background:var(--md-surface); border:1px solid var(--md-outline-variant); border-radius:16px; transition:box-shadow .2s, border-color .2s; }
      .md-card-hover:hover { box-shadow:var(--md-shadow-2); }
      ...
      .toast {
        ...
        border-radius:8px; padding:.875rem 1rem; font-size:.8125rem; box-shadow:var(--md-shadow-3);
        white-space:pre-wrap; border-left:4px solid var(--md-inverse-primary);
        animation:md-snack .24s cubic-bezier(.2,0,0,1);
      }
      .toast.ok   { border-left-color:#7ddf9e; }
      .toast.err  { border-left-color:#ffb4ab; }
      .toast.warn { border-left-color:#ffcf8a; }
...
<body class="min-h-screen" style="background:var(--md-surface);color:var(--md-on-surface);-webkit-font-smoothing:antialiased">
  ...
  <main style="max-width:1280px;margin:0 auto;padding:2rem 1.5rem 3rem">
    ...
    <header class="flex items-center justify-between mb-8">
      ...
      <div style="display:flex;align-items:center;gap:.625rem;position:relative">
        <button type="button" onclick="gerarRelatorio()" class="secondary"><span class="md-icon" style="font-size:18px">picture_as_pdf</span>Relatório</button>
        <div style="position:relative">
          ...
          <div id="import-menu" style="display:none;position:absolute;right:0;top:calc(100% + .625rem);width:280px;background:var(--md-surface-container-high);border-radius:20px;box-shadow:var(--md-shadow-3);padding:1.125rem;z-index:100">
            ...
            <div style="display:flex;flex-direction:column;gap:.75rem">
              ...
              <button type="button" onclick="importarArquivoExcel()" class="btn-primary" style="width:100%">Importar Excel</button>
  ...
  <!-- == Toast / Snackbar == -->
  <div id="toast-area" style="position:fixed;bottom:1rem;left:50%;transform:translateX(-50%);display:flex;flex-direction:column;gap:.5rem;z-index:50;width:420px;max-width:calc(100vw - 2rem)"></div>
...
function gerarRelatorioPdf() {
  ...
  var inad = todos.filter(function(i){ return (i.duplicatas_vencidas||0) > 1 && i.status_contato !== "FINALIZADO" && i.status_cadastro !== "DESCONSIDERADO"; });
  var desc = todos.filter(function(i){ return i.status_cadastro === "DESCONSIDERADO" && i.status_contato !== "FINALIZADO" && (i.duplicatas_vencidas||0) > 1; });
  var fin  = todos.filter(function(i){ return i.status_contato === "FINALIZADO"; });
  var porValor = function(a,b){ return (b.valor_total||0) - (a.valor_total||0); };
  inad.sort(porValor); desc.sort(porValor); fin.sort(porValor);

  function esc(s){ return String(s==null?"":s).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"); }
  function moeda(n){ n = Math.round(Number(n||0)*100)/100; var neg = n<0; n = Math.abs(n); var s = n.toFixed(2).split("."); var x = ""; for (var i=0;i<s[0].length;i++){ if (i>0 && (s[0].length-i)%3===0) x += "."; x += s[0].charAt(i); } return (neg?"-":"") + "R$ " + x + "," + s[1]; }
  function dbr(iso){ if(!iso) return ""; var p = String(iso).substring(0,10).split("-"); return p.length===3 ? p[2]+"/"+p[1]+"/"+p[0] : String(iso); }
  function contatoTxt(i){ var s = i.status_contato || "EM ABERTO"; if (s==="AGENDADO" && i.data_agendamento_cliente) s += " (" + dbr(i.data_agendamento_cliente) + ")"; return s; }
  function cadastroTxt(i){ var s = i.status_cadastro || ""; if ((s==="PROTESTADO"||s==="DESCONSIDERADO") && i.data_status_cadastro_cliente) s += " (" + dbr(i.data_status_cadastro_cliente) + ")"; return s || "-"; }

  function linha(i, idx){
    return "<tr>" +
      "<td class='c'>" + idx + "</td>" +
      "<td><b>" + esc(i.cliente) + "</b><br><span class='doc'>" + esc(i.cnpj_cpf) + "</span></td>" +
      "<td>" + esc(contatoTxt(i)) + "</td>" +
      "<td>" + esc(cadastroTxt(i)) + "</td>" +
      "<td class='c'>" + (i.duplicatas_vencidas||0) + "</td>" +
      "<td class='r'>" + moeda(i.valor_total||0) + "</td>" +
      "<td class='obs'>" + esc((i.observacao_cliente||"").substring(0,90)) + "</td>" +
    "</tr>";
  }

  function tabela(titulo, lista, cor){
    if (!lista.length) return "";
    var total = lista.reduce(function(s,i){ return s + (i.valor_total||0); }, 0);
    var rows = lista.map(function(i,idx){ return linha(i, idx+1); }).join("");
    return "<div class='sec' style='border-left:3px solid " + cor + "'>" + esc(titulo) + " - " + lista.length + " cliente(s) &middot; " + moeda(total) + "</div>" +
      "<table><thead><tr><th class='c'>#</th><th>Cliente</th><th>Contato</th><th>Cadastro</th><th class='c'>Dup.</th><th class='r'>Valor</th><th>Observa&ccedil;&atilde;o</th></tr></thead><tbody>" + rows + "</tbody></table>";
  }

  var agora = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "dd/MM/yyyy HH:mm");
  var totVenc = (kpis.total_vencido_ajustado !== undefined) ? kpis.total_vencido_ajustado : (kpis.total_vencido || 0);

  var baseContato = todos.filter(function(i){ return (i.duplicatas_vencidas||0) > 1 && i.status_contato !== "FINALIZADO" && i.status_cadastro !== "DESCONSIDERADO"; });
  var baseCadastro = todos.filter(function(i){ return (i.duplicatas_vencidas||0) > 1 && i.status_contato !== "FINALIZADO"; });
  function agregaStatus(lista, modo, chave){ var n=0, v=0; lista.forEach(function(i){ var val = (modo==="contato") ? (i.status_contato||"EM ABERTO") : (i.status_cadastro||""); if (val===chave){ n++; v += (i.valor_total||0); } }); return { n:n, v:v }; }
  function cardStatus(k, a, cls){ return "<div class='kpi" + (cls?(" "+cls):"") + "'><div class='l'>" + k + " (" + a.n + ")</div><div class='v'>" + moeda(a.v) + "</div></div>"; }
  function cardsStatus(){
    var out = "";
    ["EM ABERTO","EM CONTATO","AGENDADO"].forEach(function(k){ out += cardStatus(k, agregaStatus(baseContato,"contato",k), ""); });
    ["SUSPENSO","CANCELADO","PROTESTADO","PERMUTA","DESCONSIDERADO"].forEach(function(k){ var a=agregaStatus(baseCadastro,"cadastro",k); if(a.n) out += cardStatus(k, a, "cad"); });
    return out;
  }

  var html = "<!DOCTYPE html><html><head><meta charset='utf-8'><style>" +
    "@page{size:A4 portrait;margin:16px} *{box-sizing:border-box} body{font-family:Arial,Helvetica,sans-serif;color:#1a1b20;margin:0;padding:0;font-size:8px}" +
    "h1{font-size:14px;margin:0 0 1px} .meta{font-size:8px;color:#666;margin:0 0 6px}" +
    ".kpis{display:flex;gap:4px;margin:0 0 7px;flex-wrap:wrap}" +
    ".kpi{border:1px solid #dcdce4;border-radius:5px;padding:3px 7px;min-width:92px}" +
    ".kpi.tv{border-color:#f1b0ab;background:#fff5f5} .kpi.cad{background:#f7f8fc}" +
    ".kpi .l{font-size:6.5px;color:#666;text-transform:uppercase;letter-spacing:.03em} .kpi .v{font-size:10.5px;font-weight:bold;margin-top:1px}" +
    ".sec{font-size:9px;font-weight:bold;margin:6px 0 2px;padding-left:6px}" +
    "table{width:100%;border-collapse:collapse;margin-bottom:2px;table-layout:fixed}" +
    "th{background:#f1f2f8;text-align:left;padding:1.8px 5px;font-size:6.5px;text-transform:uppercase;color:#555;border-bottom:1px solid #ccc}" +
    "td{padding:1.5px 5px;border-bottom:1px solid #eee;vertical-align:top;word-wrap:break-word}" +
    ".c{text-align:center;width:18px} .r{text-align:right;white-space:nowrap;width:70px} .doc{color:#777;font-size:6.5px} .obs{color:#444}" +
    "tr:nth-child(even) td{background:#fafafa}" +
    "</style></head><body>" +
    "<h1>Relat&oacute;rio de Cobran&ccedil;as</h1>" +
    "<p class='meta'>Gerado em " + agora + " &middot; vers&atilde;o " + APP_VERSION + "</p>" +
    "<div class='kpis'>" +
      "<div class='kpi tv'><div class='l'>Total vencido</div><div class='v' style='color:#ba1a1a'>" + moeda(totVenc) + "</div></div>" +
      cardsStatus() +
    "</div>" +
    tabela("INADIMPLENTES", inad, "#0b57d0") +
    tabela("DESCONSIDERADOS", desc, "#9aa0aa") +
    tabela("PROTESTADOS", fin, "#146c43") +
    "</body></html>";

  var blob = Utilities.newBlob(html, "text/html", "relatorio.html").getAs("application/pdf");
  var fname = "relatorio_cobrancas_" + Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "yyyy-MM-dd") + ".pdf";
