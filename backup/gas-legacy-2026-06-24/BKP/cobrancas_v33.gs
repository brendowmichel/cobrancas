// SISTEMA DE GESTAO DE COBRANCAS - Apps Script v33
...
<body class="min-h-screen" style="background:var(--md-surface);color:var(--md-on-surface);-webkit-font-smoothing:antialiased">
  ...
  <script>
    ...
    function renderGraficos(inadimplentes) {
      ...
      if (el2) el2.innerHTML = buildGrafico_("status_cadastro", "STATUS DE CADASTRO", totalCadastro + " com status especial (de " + total + ")", CADASTRO_DEFS, cadastroCounts, cadastroTotals);
    }
    ...
    function buildGrafico_(campo, titulo, subtitulo, defs, counts, totals) {
      ...
      var legendHtml = defs.map(function(d) {
        ...
        var wrapStyle = "display:flex;align-items:flex-start;gap:5px;cursor:pointer;padding:3px 7px;border-radius:8px;transition:background .15s,opacity .15s,box-shadow .15s"
          ...
          + (dim ? ";opacity:.4" : "");
        var hover = ativo ? "" : " onmouseover=\"this.style.background='var(--md-surface-container-high)'\" onmouseout=\"this.style.background='transparent'\"";
        return "<div role='button' tabindex='0' onclick=\"toggleFiltroCategoria('" + campo + "','" + escapeAttr(d.key) + "')\" title='Filtrar lista por " + escapeHtml(d.key) + "' style='" + wrapStyle + "'" + hover + ">" +
