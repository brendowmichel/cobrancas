// SISTEMA DE GESTAO DE COBRANCAS - Apps Script v43
...
<head>
  ...
  <style type="text/tailwindcss">
    ...
    @layer components {
      ...
      .priority-tag {
        @apply inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold uppercase;
      ...
      .quick-filters { display:flex; align-items:center; gap:.5rem; flex-wrap:wrap; }
      .quick-filter {
        height:32px; padding:0 .875rem; border-radius:999px; border:1px solid var(--md-outline-variant);
        background:var(--md-surface-container-lowest); color:var(--md-on-surface-variant);
        font-size:.75rem; font-weight:600; cursor:pointer; transition:background .15s,border-color .15s,color .15s;
      }
      .quick-filter:hover { background:var(--md-surface-container); color:var(--md-primary); }
      .quick-filter.active { background:var(--md-primary-container); color:var(--md-on-primary-container); border-color:#a8c7fa; }
...
<body class="min-h-screen" style="background:var(--md-surface-container-low);color:var(--md-on-surface);-webkit-font-smoothing:antialiased">
  ...
  <main style="max-width:1280px;margin:0 auto;padding:2rem 1.5rem 3rem">
    ...
    <div class="md-card" style="overflow:hidden">
      ...
      <div style="padding:.75rem 1.25rem;border-bottom:1px solid var(--md-outline-variant)">
        ...
        <div id="quick-filters" class="quick-filters" style="margin-top:.75rem"></div>
      </div>
  ...
  <script>
    ...
    const QUICK_FILTERS = [
      { label: "Todos", campo: "", valor: "" },
      { label: "Em aberto", campo: "status_contato", valor: "EM ABERTO" },
      { label: "Em contato", campo: "status_contato", valor: "EM CONTATO" },
      { label: "Agendado", campo: "status_contato", valor: "AGENDADO" },
      { label: "Suspenso", campo: "status_cadastro", valor: "SUSPENSO" },
      { label: "Permuta", campo: "status_cadastro", valor: "PERMUTA" }
    ];
    ...
    function renderQuickFilters() {
      ...
      if (!el) return;
      const base = inadimplentesView || [];
      el.innerHTML = QUICK_FILTERS.map(function (f) {
        var active = (!f.campo && !filtroCategoria) ||
          (f.campo && filtroCategoria && filtroCategoria.campo === f.campo && filtroCategoria.valor === f.valor);
        var count = "";
        if (f.campo) {
          count = base.filter(function (i) {
            return f.campo === "status_contato"
              ? (i.status_contato || "EM ABERTO") === f.valor
              : (i.status_cadastro || "") === f.valor;
          }).length;
        } else {
          count = base.length;
        }
        return "<button type='button' class='quick-filter" + (active ? " active" : "") + "' onclick='setFiltroRapido(&quot;" + escapeAttr(f.campo) + "&quot;,&quot;" + escapeAttr(f.valor) + "&quot;)'>" +
          escapeHtml(f.label) + " " + count +
        "</button>";
      }).join("");
    }

    function setFiltroRapido(campo, valor) {
      filtroCategoria = campo ? { campo: campo, valor: valor } : null;
      var _f = document.getElementById("filtro-cliente");
      filtrarInadimplentes(_f ? _f.value : "");
      renderGraficos(dashboardData ? (dashboardData.inadimplentes || []) : []);
      renderQuickFilters();
    ...
    function clienteNomeHtml(item) {
      ...
      const tag = item.status_cadastro
        ...
        : "";
      const prioridade = isAgendamentoVencido(item)
        ? " <span class='priority-tag' title='Agendamento vencido'>AG. VENCIDO</span>"
        : "";
      return "<span class='client-name'><strong>" + escapeHtml(item.cliente) + "</strong>" + tag + prioridade + "</span>";
    }
