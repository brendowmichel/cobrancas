// SISTEMA DE GESTAO DE COBRANCAS - Apps Script v37
  ...
  <main style="max-width:1280px;margin:0 auto;padding:2rem 1.5rem 3rem">
    ...
    <div class="md-card" style="overflow:hidden">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;padding:1rem 1.25rem;border-bottom:1px solid var(--md-outline-variant)">
        <div>
          <h2 style="font-size:1.125rem;font-weight:600;color:var(--md-on-surface);margin:0">Inadimplentes</h2>
    ...
    <div id="finalizados-section" style="display:none;margin-top:1.5rem;overflow:hidden" class="md-card">
      <div style="display:flex;align-items:center;justify-content:space-between;padding:1rem 1.25rem;border-bottom:1px solid var(--md-outline-variant)">
        <div>
          <h2 style="font-size:1.125rem;font-weight:600;color:var(--md-on-surface);margin:0">Finalizados</h2>
...
function gerarRelatorioPdf() {
  ...
  var html = "<!DOCTYPE html><html><head><meta charset='utf-8'><style>" +
    "*{box-sizing:border-box} body{font-family:Arial,Helvetica,sans-serif;color:#1a1b20;margin:0;padding:14px;font-size:9px}" +
    "h1{font-size:16px;margin:0 0 2px} .meta{font-size:9px;color:#666;margin:0 0 9px}" +
    ".kpis{display:flex;gap:7px;margin:0 0 10px;flex-wrap:wrap}" +
    ".kpi{border:1px solid #dcdce4;border-radius:6px;padding:5px 10px}" +
    ".kpi .l{font-size:7.5px;color:#666;text-transform:uppercase;letter-spacing:.04em} .kpi .v{font-size:13px;font-weight:bold}" +
    ".sec{font-size:10px;font-weight:bold;margin:10px 0 3px;padding-left:6px}" +
    "table{width:100%;border-collapse:collapse;margin-bottom:4px;table-layout:fixed}" +
    "th{background:#f1f2f8;text-align:left;padding:3px 5px;font-size:7.5px;text-transform:uppercase;color:#555;border-bottom:1px solid #ccc}" +
    "td{padding:2.5px 5px;border-bottom:1px solid #eee;vertical-align:top;word-wrap:break-word}" +
    ".c{text-align:center;width:22px} .r{text-align:right;white-space:nowrap;width:78px} .doc{color:#777;font-size:8px} .obs{color:#444}" +
    "tr:nth-child(even) td{background:#fafafa}" +
    "</style></head><body>" +
    "<h1>Relat&oacute;rio de Cobran&ccedil;as</h1>" +
    "<p class='meta'>Gerado em " + agora + " &middot; vers&atilde;o " + APP_VERSION + "</p>" +
    "<div class='kpis'>" +
      "<div class='kpi'><div class='l'>Total vencido</div><div class='v' style='color:#ba1a1a'>" + moeda(totVenc) + "</div></div>" +
      "<div class='kpi'><div class='l'>Inadimplentes</div><div class='v'>" + inad.length + "</div></div>" +
      "<div class='kpi'><div class='l'>Duplicatas vencidas</div><div class='v'>" + (kpis.duplicatas_vencidas||0) + "</div></div>" +
      "<div class='kpi'><div class='l'>Maior atraso</div><div class='v'>" + (kpis.maior_atraso||0) + " dias</div></div>" +
      "<div class='kpi'><div class='l'>Desconsiderados</div><div class='v'>" + desc.length + "</div></div>" +
    "</div>" +
    tabela("INADIMPLENTES", inad, "#0b57d0") +
    tabela("DESCONSIDERADOS", desc, "#9aa0aa") +
    tabela("FINALIZADOS", fin, "#146c43") +
    "</body></html>";
